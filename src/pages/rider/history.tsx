  ...
  return (
    <MainLayout>
      <div class="page-section">
        <div>
          <h1 class="heading-page text-2xl font-bold text-text-primary">Ride History</h1>
          <p class="text-text-secondary mt-2 leading-relaxed">View all your past rides</p>
        </div>
        ...
        <Show when={filtered().length > 0} fallback={
          <EmptyState
            ...
            description={filter() === 'all' ? "You haven't taken any rides yet. Book your first trip to see it here." : `No ${filter().replace('_', ' ')} rides match this filter.`}
            actionLabel="Book a Ride"
            onAction={() => navigate('/rider/book')}
          />
        }>
          <div class="space-y-4">
                    ...
                    <Show when={ride.rating}>
                      <div class="flex items-center gap-1 justify-end mt-1">
                        <For each={Array(5).fill(0)}>{(_, i) => (
                          <svg class={`w-3 h-3 ${i() < (ride.rating ?? 0) ? 'text-warning' : 'text-text-muted'}`} fill="currentColor" viewBox="0 0 20 20">
        ...
        <Show when={selected()}>
          <div class="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-overlay" onClick={() => setSelected(null)}>
            <div class="w-full sm:max-w-lg glass-dropdown rounded-t-2xl sm:rounded-2xl p-8 animate-slide-in-up" onClick={(e) => e.stopPropagation()}>
