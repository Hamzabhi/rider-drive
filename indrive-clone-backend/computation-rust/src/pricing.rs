...
/// Estimate trip duration from distance + assumed speed.
...
pub fn compute_fare(
    ...
    // distance + duration
    let distance_m = explicit_distance_m
        .unwrap_or_else(|| haversine_m(pickup_lat, pickup_lng, dropoff_lat, dropoff_lng));
    ...
    let base_fare = rule.base;
    let distance_fare = distance_km * rule.per_km;
    let time_fare = minutes * rule.per_min;
    let booking_fee = rule.booking_fee;
    ...
    let raw_total = (base_fare + distance_fare + time_fare) * surge + booking_fee;
    let suggested = raw_total.max(rule.minimum);

    ComputedFare {
        base_fare,
        distance_fare: distance_fare * surge,
        time_fare: time_fare * surge,
        surge_multiplier: surge,
        booking_fee,
        suggested_fare: suggested,
        minimum_fare: rule.minimum,
        distance_m,
        duration_s,
    }
}
